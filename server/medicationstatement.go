...
func MedicationStatementIndexHandler(rw http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	...
	if len(r.Form) == 0 {
		...
		query := search.Query{Resource: "MedicationStatement", Query: r.URL.RawQuery}
	...
	var medicationstatementEntryList []models.BundleEntryComponent
	for i := range result {
...
func MedicationStatementCreateHandler(rw http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	...
	json.NewEncoder(rw).Encode(medicationstatement)
}
